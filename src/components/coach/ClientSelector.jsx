    ...
    <div className="w-full md:w-64">
      ...
      >
        ...
        <SelectContent className="bg-[#1a1a20] border-gray-800 text-white">
          <SelectItem value="me" className="font-bold text-[#7b2cbf]">{t('dashboard')}</SelectItem>
          ...
          {clients.map(client => (
            <SelectItem key={client.id} value={client.id} className="focus:bg-[#00f5d4] focus:text-black">
                ...
                <span className="font-bold">
                    {client.full_name || (client.firstName ? `${client.firstName} ${client.lastName}` : client.email)}
                </span>
            </SelectItem>
          ))}
        </SelectContent>
