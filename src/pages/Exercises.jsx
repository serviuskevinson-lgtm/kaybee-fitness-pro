...
export default function Exercises() {
  ...
  const handleSaveTemplate = async () => {
    ...
    setIsSaving(true);
    try {
      const templateData = { name: newTemplateName, exercises: cart, createdBy: isCoachView ? 'coach' : 'client', createdAt: serverTimestamp() };
  ...
  const handleProgramWeek = async () => {
      if (!programName.trim() || programDates.length === 0) { alert("Donne un nom et choisis au moins une date."); return; }
      setIsSaving(true);
      try {
          const workoutData = { id: Date.now().toString(), name: programName, exercises: cart, scheduledDays: programDates.map(d => format(d, 'yyyy-MM-dd')), createdAt: new Date().toISOString(), assignedBy: isCoachView ? 'coach' : 'self' };
          const userRef = doc(db, "users", targetUserId);
          await updateDoc(userRef, { workouts: arrayUnion(workoutData) });
    ...
    try {
      ...
      const workoutPromises = generatedWorkouts.map(async (workout) => {
        const enhancedExercises = workout.exercises.map(ex => {
          const found = exercisesDB.find(dbEx =>
            ...
            try {
                await WearPlugin.sendDataToWatch({
                    ...
                    data: JSON.stringify({
                        ...
                        health: { today_workout: workoutData }
                    })
                });
            } catch (e) { console.log("Watch sync skipped"); }
        }
  ...
  const handleCreateExercise = async () => {
      ...
      try {
          ...
          const newExo = { name: newExoData.name, group: newExoData.group, equipment: newExoData.equipment, description: newExoData.description, imageUrl: downloadUrl, mediaType: isVideo ? 'video' : 'image', authorId: currentUser.uid, authorName: currentUser.displayName || "Coach", targetMuscles: [newExoData.group], createdAt: serverTimestamp() };
                ...
                <div className="flex space-x-4">
                    {templates.map((tpl) => (
                        <div key={tpl.id} onClick={() => setTemplateToLoad(tpl)} className={`relative group flex-shrink-0 w-72 p-5 rounded-2xl border cursor-pointer transition-all duration-300 hover:scale-[1.02] hover:shadow-lg ${tpl.createdBy === 'coach' ? 'bg-gradient-to-br from-[#7b2cbf]/20 to-[#9d4edd]/10 border-[#7b2cbf]/50 hover:border-[#9d4edd]' : 'bg-[#1a1a20] border-gray-700 hover:border-[#9d4edd]'}`}>
                            ...
                            <div className="flex items-center justify-between text-xs text-gray-400 mb-4"> <span className="flex items-center gap-1"><LayoutList size={12}/> {tpl.exercises?.length || 0} Exos</span> <span>{tpl.createdAt?.seconds ? new Date(tpl.createdAt.seconds * 1000).toLocaleDateString() : ""}</span> </div>
                        ...
                        return (
                            <div key={exo.id} className={`rounded-2xl border transition-all group overflow-hidden flex flex-col h-full ${exo.isCustom ? 'bg-[#1a1a25] border-[#9d4edd]/30 hover:border-[#9d4edd] hover:shadow-[0_0_20px_rgba(157,78,221,0.2)]' : 'bg-[#1a1a20] border-gray-800 hover:border-[#9d4edd]'}`}>
                                ...
                                <div className="p-4 flex-1 flex flex-col justify-between">
                                    ...
                                    <Button size="sm" className={`w-full mt-3 h-9 font-bold text-xs rounded-lg transition-all ${isAdded ? 'bg-red-500/10 text-red-500 hover:bg-red-500/20 border border-red-500/50' : 'bg-[#9d4edd] text-white hover:bg-[#7b2cbf] shadow-lg shadow-purple-500/20'}`} onClick={() => isAdded ? removeFromCart(isAdded.uniqueId) : addToCart(exo)}> {isAdded ? <><Trash2 size={14} className="mr-1"/> {t('remove')}</> : <><Plus size={14} className="mr-1"/> {t('add')}</>} </Button>
                                </div>
                            </div>
                        );
                    })}
        ...
        <div className="lg:col-span-1">
            <Card className="bg-[#1a1a20] border-gray-800 sticky top-4 h-[calc(100vh-100px)] flex flex-col shadow-[0_0_30px_rgba(0,0,0,0.5)] rounded-3xl overflow-hidden ring-1 ring-white/5">
                <div className="p-6 border-b border-gray-800 bg-gradient-to-b from-[#252530] to-[#1a1a20]">
                    <div className="flex justify-between items-center mb-1"> <h2 className="font-black text-xl text-white flex items-center gap-2 italic uppercase"><LayoutList className="text-[#9d4edd]"/> {t('current_session')}</h2> <Badge className="bg-[#9d4edd] text-white font-bold px-2.5 py-0.5 text-xs">{cart.length}</Badge> </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                    {cart.map((item, index) => (
                        <div key={item.uniqueId} className="bg-[#0f0f13] p-4 rounded-2xl border border-gray-800 hover:border-[#9d4edd]/50 transition-colors group animate-in slide-in-from-right duration-300">
                            <div className="flex justify-between items-start mb-3"> <span className="font-bold text-sm text-white flex gap-3 items-center"><span className="w-5 h-5 rounded-full bg-[#9d4edd] text-white flex items-center justify-center text-[10px] font-black">{index + 1}</span> <span className="truncate w-40">{item.name}</span></span> <button onClick={() => removeFromCart(item.uniqueId)} className="text-gray-600 hover:text-red-500 transition-colors p-1 hover:bg-red-500/10 rounded-md"><Trash2 size={14}/></button> </div>
                    ...
                    {cart.length === 0 && ( <div className="text-center text-gray-500 py-10 italic flex flex-col items-center justify-center h-full"> <div className="w-20 h-20 rounded-full bg-gray-800/30 flex items-center justify-center mb-4 border border-dashed border-gray-700"><Plus className="text-gray-600" size={32}/></div> <p className="font-bold text-gray-400">{t('session_empty')}</p> <p className="text-xs mt-2 text-gray-600">{t('add_from_left')}</p> </div> )}
                </div>
                <div className="p-5 border-t border-gray-800 bg-[#15151a] space-y-3">
                    <Button disabled={cart.length === 0} onClick={() => setIsSaveModalOpen(true)} className="w-full bg-[#1a1a20] border border-gray-700 hover:bg-gray-800 text-white font-bold h-12 rounded-xl"> <Save className="mr-2 h-4 w-4" /> {t('save_favorite')} </Button>
                    <Button disabled={cart.length === 0} onClick={() => setIsProgramModalOpen(true)} className="w-full bg-[#7b2cbf] hover:bg-[#9d4edd] text-white font-bold h-12 rounded-xl"> <CalendarIcon className="mr-2 h-4 w-4" /> Programmer </Button>
                    <Button className={`w-full font-black text-white h-14 text-lg rounded-xl shadow-lg hover:shadow-xl transition-all hover:scale-[1.02] ${isCoachView ? 'bg-gradient-to-r from-[#7b2cbf] to-[#9d4edd]' : 'bg-gradient-to-r from-[#00f5d4] to-[#00b89f] text-black'}`} disabled={cart.length === 0} onClick={() => navigate('/session', { state: { sessionData: cart } })}> {isCoachView ? t('assign_to_client') : t('start')} </Button>
                </div>
            </Card>
        </div>
      </div>
            ...
            {autoBuildStep === 1 ? (
                <div className="space-y-6 py-4 animate-in fade-in duration-300 flex flex-col items-center">
                    ...
                    <div className="w-full bg-black/40 p-4 rounded-3xl border border-white/5">
                        <Calendar mode="multiple" selected={autoBuildDates} onSelect={setAutoBuildDates} locale={fr} className="bg-transparent text-white" />
                    </div>
      ...
      <Dialog open={isProgramModalOpen} onOpenChange={setIsProgramModalOpen}>
        <DialogContent className="bg-[#1a1a20] border-gray-800 text-white rounded-3xl max-w-md">
            ...
            <div className="space-y-6 py-4 flex flex-col items-center">
                ...
                <div className="w-full bg-black/40 p-4 rounded-3xl border border-white/5">
                    <Calendar mode="multiple" selected={programDates} onSelect={setProgramDates} locale={fr} className="bg-transparent text-white" />
                </div>
            </div>
            <DialogFooter><Button onClick={handleProgramWeek} className="w-full bg-[#00f5d4] text-black font-bold h-12 rounded-xl">Confirmer la planification</Button></DialogFooter>
        </DialogContent>
      </Dialog>
