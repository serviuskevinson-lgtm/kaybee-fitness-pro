    ...
    <div className="p-4 lg:p-8 min-h-screen bg-[#0a0a0f] text-white relative pb-32">
      ...
      <div className="max-w-7xl mx-auto mb-6">
        <div className="bg-black/40 p-1 rounded-2xl border border-gray-800 shadow-2xl flex gap-1">
            <button
                ...
                className={`flex-1 h-12 rounded-xl font-black uppercase italic tracking-tighter text-sm transition-all duration-300 flex items-center justify-center gap-2 border
                    ${activeTab === 'gallery'
      ...
      {activeTab === 'gallery' ? (
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <div className={`p-2.5 rounded-xl ${isCoachView ? 'bg-[#7b2cbf]' : 'bg-[#00f5d4]'}`}>
                ...
                <div className="flex gap-2">
                    <Button
                        variant="outline"
                        size="icon"
                        className={`transition-all h-10 w-10 rounded-xl border-2 ${
                            compareMode
                    ...
                    <Button
                        size="icon"
                        className={`h-10 w-10 rounded-xl shadow-xl transition-all
                ...
                >
                    <Card className="bg-[#1a1a20] border border-gray-800 shadow-2xl rounded-2xl overflow-hidden mb-6">
                        <CardContent className="p-4 space-y-4">
                            <div
                                className="border-2 border-dashed border-gray-700 rounded-2xl flex flex-col items-center justify-center p-6 cursor-pointer hover:border-[#00f5d4] hover:bg-[#00f5d4]/5 transition-all min-h-[180px] group relative overflow-hidden"
                                ...
                                {uploadFiles.length > 0 ? (
                                    <div className="w-full">
                                        <div className="flex justify-between items-center mb-4">
                                            <p className="text-[#00f5d4] font-black uppercase italic tracking-tighter text-sm">{uploadFiles.length} MÉDIAS PRÊTS</p>
                                            <div className="flex gap-2">
                                                {uploadFiles.some(f => f.type.startsWith('video')) && (
                                                    <Button size="sm" variant="ghost" onClick={(e) => { e.stopPropagation(); setIsMuted(!isMuted); }} className={`h-8 w-8 rounded-full ${isMuted ? 'text-red-500 bg-red-500/10' : 'text-[#00f5d4] bg-[#00f5d4]/10'}`}>
                                            ...
                                            {uploadFiles.map((file, idx) => (
                                                <div key={idx} className="relative aspect-square bg-black rounded-xl overflow-hidden border border-gray-800" onClick={(e) => e.stopPropagation()}>
                                                    {file.type.startsWith('video') ? (
                                                        <div className="w-full h-full flex items-center justify-center text-gray-500 bg-gray-900"><Video size={18}/></div>
                                                    ) : (
                                    ...
                                    <div className="text-center space-y-2">
                                        <div className="w-12 h-12 rounded-xl bg-gray-800 flex items-center justify-center mx-auto group-hover:bg-[#00f5d4] group-hover:text-black transition-all">
                                            <Upload size={20} />
                                        </div>
                                        <p className="text-white font-black uppercase italic tracking-tighter text-xs">{t('click_to_upload')}</p>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="space-y-3">
                                <Textarea
                                    ...
                                    placeholder="Légende..."
                                    className="bg-black/40 border-gray-800 rounded-xl min-h-[80px] text-sm focus:border-[#00f5d4] transition-all"
                                />

                                <div className="grid grid-cols-2 gap-3">
                                    <Select value={uploadCategory} onValueChange={setUploadCategory}>
                                        <SelectTrigger className="bg-black/40 border-gray-800 text-white h-10 rounded-xl text-xs"><SelectValue /></SelectTrigger>
                                        <SelectContent className="bg-[#1a1a20] border-gray-800 text-white rounded-xl">
                                            <SelectItem value="physique" className="uppercase font-bold text-[10px]">{t('physique')}</SelectItem>
                                            <SelectItem value="posing" className="uppercase font-bold text-[10px]">{t('posing')}</SelectItem>
                                            <SelectItem value="repas" className="uppercase font-bold text-[10px]">{t('meal')}</SelectItem>
                                            <SelectItem value="reference" className="uppercase font-bold text-[10px]">Suivi</SelectItem>
                                        </SelectContent>
                                    </Select>
                                    <Select value={uploadPrivacy} onValueChange={(val) => { setUploadPrivacy(val); setModerationError(null); }}>
                                        <SelectTrigger className="bg-black/40 border-gray-800 text-white h-10 rounded-xl text-xs"><SelectValue /></SelectTrigger>
                                        <SelectContent className="bg-[#1a1a20] border-gray-800 text-white rounded-xl">
                                            <SelectItem value="Coach" className="uppercase font-bold text-[10px]">{t('visible_coach')}</SelectItem>
                                            <SelectItem value="Privé" className="uppercase font-bold text-[10px]">{t('me_only')}</SelectItem>
                                            <SelectItem value="Public" className="uppercase font-bold text-[10px]">{t('visible_community')}</SelectItem>
                                        </SelectContent>
                                    </Select>
                                </div>

                                <Button
                                    className={`w-full font-black uppercase italic tracking-tighter text-md h-12 rounded-xl shadow-xl transition-all ${isCoachView ? 'bg-[#7b2cbf] text-white hover:bg-[#9d4edd]' : 'bg-[#00f5d4] text-black'}`}
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 custom-scrollbar">
              {CATEGORIES.map(cat => (
                  <button
                    ...
                    className={`flex-shrink-0 h-9 px-4 rounded-full text-[10px] font-black uppercase italic tracking-widest transition-all border flex items-center gap-2
          ...
          {compareMode && (
              <div className="bg-[#00f5d4]/10 border border-[#00f5d4]/40 px-4 py-2 rounded-xl flex items-center justify-between animate-in slide-in-from-top">
                  <span className="text-[10px] font-black text-[#00f5d4] uppercase tracking-widest">Comparer: {compareSelection.length}/2</span>
                  {compareSelection.length === 2 && (
                      <Button size="sm" className="h-8 px-4 bg-[#00f5d4] text-black font-black uppercase italic rounded-lg" onClick={() => setSelectedMedia('COMPARE')}>
                          Analyser
                      </Button>
                  )}
              </div>
                          ...
                          return (
                              ...
                              >
                                  ...
                                  <div className="absolute bottom-2 left-2 right-2 flex justify-between items-end">
                                      <p className="text-white font-black italic uppercase tracking-tighter text-[9px]">{format(new Date(media.createdAt), 'dd MMM', {locale: fr})}</p>
                                      {media.type === 'video' && <Play size={10} fill="white" className="text-white"/>}
                                  </div>

                                  {compareMode && isSelectedForCompare && (
                                      <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                                          <div className="bg-[#00f5d4] text-black w-10 h-10 rounded-full font-black italic text-lg flex items-center justify-center border-2 border-black shadow-lg">
      ...
      <Dialog open={!!selectedMedia} onOpenChange={() => { setSelectedMedia(null); if(selectedMedia === 'COMPARE') setCompareSelection([]); }}>
        <DialogContent className="bg-[#0a0a0f] border-gray-800 text-white max-w-full p-0 overflow-hidden rounded-t-[2rem] md:rounded-[2rem] bottom-0 md:bottom-auto fixed md:relative max-h-[90vh]">
            {selectedMedia === 'COMPARE' ? (
                    ...
                    <div className="flex-1 grid grid-cols-2 gap-0.5 bg-black relative">
                        {compareSelection.map((item, idx) => (
                            <div key={idx} className="relative h-full flex items-center justify-center overflow-hidden border-r border-white/5 last:border-none">
                                ...
                                <div className="absolute top-4 left-4 bg-black/70 backdrop-blur-lg px-3 py-1 rounded-lg text-[8px] font-black uppercase italic tracking-widest border border-white/10">
                                    {format(new Date(item.createdAt), 'dd MMM yyyy', {locale: fr})}
                                </div>
                            </div>
                        ))}
                    </div>
                    <div className="p-4 bg-[#1a1a20] flex justify-between items-center border-t border-white/5">
                        <p className="text-white font-black italic uppercase tracking-tighter text-sm">VS</p>
                        <Button onClick={() => setSelectedMedia(null)} className="bg-white/5 text-white font-black uppercase italic rounded-xl px-6 h-10 text-xs">Fermer</Button>
                    </div>
                </div>
            ) : (
                selectedMedia && selectedMedia !== 'COMPARE' && (
                    <div className="flex flex-col h-full">
                        ...
                        <div className="p-6 bg-[#1a1a20] space-y-4 max-h-[50vh] overflow-y-auto">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <Avatar className="w-10 h-10 border border-[#7b2cbf]">
                                        <AvatarFallback className="bg-[#7b2cbf] text-white font-black italic text-sm">{selectedMedia.authorName?.[0]}</AvatarFallback>
                                    </Avatar>
                                    <div>
                                        <p className="font-black italic uppercase tracking-tighter text-sm text-white">{selectedMedia.authorName}</p>
                                        <p className="text-[8px] text-gray-500 font-bold uppercase tracking-widest">{format(new Date(selectedMedia.createdAt), 'dd MMMM yyyy', {locale: fr})}</p>
                                    </div>
                                </div>
                                <PrivacyBadge privacy={selectedMedia.privacy} />
                            </div>

                            <p className="text-xs text-gray-400 italic leading-relaxed bg-black/20 p-4 rounded-2xl border border-white/5">
                              {selectedMedia.caption || "Aucune description."}
                            </p>

                            <div className="flex gap-2 pt-2">
                                <Button variant="ghost" className="flex-1 h-12 rounded-xl text-gray-500 hover:text-red-500 font-black uppercase italic hover:bg-red-500/10" onClick={() => handleDelete(selectedMedia.id)}>
                                    <Trash2 size={18} className="mr-2" /> Supprimer
                                </Button>
                                <Button variant="ghost" className="flex-1 h-12 rounded-xl text-gray-500 hover:text-[#00f5d4] font-black uppercase italic hover:bg-[#00f5d4]/10">
