...
// --- CONSTANTES ---
const CATEGORIES = [
  { id: 'all', label: 'Tout', icon: <LayoutGrid size={14}/> },
  { id: 'physique', label: 'Physique', icon: <Users size={14}/> },
  { id: 'posing', label: 'Posing', icon: <Camera size={14}/> },
  { id: 'repas', label: 'Repas', icon: <ImageIcon size={14}/> },
  { id: 'reference', label: 'Coach Ref', icon: <UserCheck size={14}/> }
];
...
export default function Gallery() {
  ...
  const PrivacyBadge = ({ privacy }) => {
      const config = {
          'Public': { icon: Globe, color: 'text-green-400', bg: 'bg-green-400/10', border: 'border-green-400/20' },
          'Amis': { icon: Users, color: 'text-blue-400', bg: 'bg-blue-400/10', border: 'border-blue-400/20' },
          'Coach': { icon: UserCheck, color: 'text-[#9d4edd]', bg: 'bg-[#9d4edd]/10', border: 'border-[#9d4edd]/20' },
          'Privé': { icon: Lock, color: 'text-red-400', bg: 'bg-red-400/10', border: 'border-red-400/20' },
      }[privacy] || { icon: Lock, color: 'text-gray-400', bg: 'bg-gray-400/10', border: '' };
    ...
    <div className="p-4 lg:p-8 min-h-screen bg-[#0a0a0f] text-white relative pb-32">
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-end gap-6 mb-8">
            ...
            <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                    <h1 className="text-4xl font-black italic uppercase text-white">
                        {isCoachView ? "Galerie Client" : "Ma Galerie"}
                    </h1>
        ...
        <div className="flex gap-2">
            {/* --- BOUTON COMPARER (COULEUR CYAN) --- */}
            <Button 
                variant="outline" 
                className={`font-bold transition-all ${
                    compareMode 
                    ? 'bg-[#00f5d4] text-black border-[#00f5d4] hover:bg-[#00f5d4]/80' // Actif: Cyan Vif
                    : 'border-gray-700 text-gray-300 hover:border-[#00f5d4] hover:text-[#00f5d4] hover:bg-[#00f5d4]/10' // Inactif: Gris -> Cyan
                }`}
                onClick={() => { setCompareMode(!compareMode); setCompareSelection([]); }}
            >
                <SplitSquareHorizontal className="mr-2 h-4 w-4" /> 
                {compareMode ? "Terminer" : "Comparer"}
            </Button>
            
            <Button 
                className={`font-black text-black ${isCoachView ? 'bg-[#7b2cbf] text-white hover:bg-[#9d4edd]' : 'bg-[#00f5d4] hover:bg-[#00f5d4]/80'}`}
                onClick={() => setIsUploadOpen(!isUploadOpen)}
            >
                <Upload className="mr-2 h-4 w-4" /> 
                {isUploadOpen ? "Fermer" : "Ajouter"}
            </Button>
        </div>
      </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            >
                                ...
                                {uploadFile ? (
                                    ...
                                    <div className="text-center">
                                        ...
                                        <p className="text-gray-300 font-bold">Cliquez pour upload</p>
                            ...
                            <div className="md:col-span-2 space-y-4">
                                <h3 className="font-bold text-white flex items-center gap-2">
                                    <Camera size={16} className="text-[#00f5d4]"/> Détails du post
                                </h3>
                                
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs text-gray-500 uppercase font-bold mb-1 block">Catégorie</label>
                                        <Select value={uploadCategory} onValueChange={setUploadCategory}>
                                            <SelectTrigger className="bg-black border-gray-700 text-white">
                                                <SelectValue />
                                            </SelectTrigger>
                                            <SelectContent className="bg-[#1a1a20] border-gray-700 text-white">
                                                <SelectItem value="physique">Physique / Shape</SelectItem>
                                                <SelectItem value="posing">Posing / Flex</SelectItem>
                                                <SelectItem value="repas">Nutrition</SelectItem>
                                                <SelectItem value="autre">Autre</SelectItem>
                                            </SelectContent>
                                        </Select>
                                    </div>
                                    <div>
                                        <label className="text-xs text-gray-500 uppercase font-bold mb-1 block">Visibilité</label>
                                        <Select value={uploadPrivacy} onValueChange={setUploadPrivacy}>
                                            <SelectTrigger className="bg-black border-gray-700 text-white">
                                                <SelectValue />
                                            </SelectTrigger>
                                            <SelectContent className="bg-[#1a1a20] border-gray-700 text-white">
                                                <SelectItem value="Coach">Coach Uniquement</SelectItem>
                                                <SelectItem value="Privé">Privé (Moi seul)</SelectItem>
                                                <SelectItem value="Amis">Amis</SelectItem>
                                                <SelectItem value="Public">Public (Communauté)</SelectItem>
                                            </SelectContent>
                                        </Select>
                                    </div>
                                </div>
                                ...
                                >
                                    {isCoachView ? "AJOUTER COMME RÉFÉRENCE" : "PUBLIER LE SOUVENIR"}
                                </Button>
                            </div>
                        </div>
                    </CardContent>
      ...
      {activeTab === 'gallery' ? (
        <div className="max-w-7xl mx-auto">
            ...
            {filteredMedia.length === 0 && !isLoading && (
                <div className="text-center py-20 border-2 border-dashed border-gray-800 rounded-3xl bg-[#1a1a20]/50">
                    ...
                    <p className="text-gray-400 font-bold text-lg">La galerie est vide.</p>
        ...
        <div className="max-w-4xl mx-auto text-center py-20">
            <h2 className="text-2xl font-bold text-white mb-2">Flux Communautaire</h2>
            ...
            {selectedMedia === 'COMPARE' && compareSelection.length === 2 ? (
                <div className="flex flex-col h-[80vh]">
                    ...
                    <div className="p-4 bg-[#1a1a20] flex justify-between items-center border-t border-gray-800">
                        ...
                        <Button onClick={() => setSelectedMedia(null)} variant="outline" className="border-gray-700 text-white">Fermer</Button>
                    </div>
                </div>
            ) : (
